...
const login = async (req: Request, res: Response): Promise<void> => {
    try {
        ...
        const lowerCaseUsername = username.toLowerCase();
        const user = await User.findOne({username: lowerCaseUsername}).select('+password') ;
        if (!user) {
        ...
        // Generate JWT token
        const token = jwt.sign({ userId: user._id, username: user.username, role: user.role }, JWT_KEY, {
